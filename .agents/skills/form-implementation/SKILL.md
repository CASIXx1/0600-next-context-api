...
- リソース削除は確認 UI を経由してから実行する。確認 UI の表示/非表示は局所 UI state で持ち、削除 API 実行は context hook 側の handler を呼ぶ。
...
- 削除確認 UI には対象名、取り消せない旨、削除実行ボタン、キャンセルボタンを含める。削除中は実行/キャンセルボタンを disabled にして多重実行を防ぐ。
