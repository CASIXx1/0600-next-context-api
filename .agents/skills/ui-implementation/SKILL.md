...
- Atomic Design 配下の UI 部品は atoms/molecules/organisms/templates のすべてで `index.tsx` に直接コンポーネントを定義する。`src/components/atoms/Button/index.tsx` と同じ形式に揃え、`src/components/atoms/StatIcon` のように別ファイルを `index.ts` から re-export する構成にしない。
