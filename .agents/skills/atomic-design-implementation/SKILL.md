...
3. domain data や context hook を読む部分は organisms に残す。
4. route params/search params、Provider 合成、navigation は `src/app/<route>/content.tsx` に残す。
5. import path を更新し、CSS module も移動先コンポーネントと同じ粒度にする。
6. 既存 UI の見た目を変えない。Atomic Design 移行とデザイン変更を同時に行わない。
7. 移行後は `rg` で旧 import と旧ファイル参照が残っていないか確認する。
