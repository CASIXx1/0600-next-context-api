...
- CSS module はコンポーネント所有で同じ粒度に置く。Atomic Design 配下では `index.tsx` と同階層の `index.module.css` に置く。
- Atomic Design 配下の新規作成・移行では `Button.module.css` や `ConfirmPanel.module.css` のようなコンポーネント名 CSS module は使わない。
...
3. 選んだ atom/molecule と同じ粒度の CSS module は同一作業で移動してよい。移動先では `index.module.css` に揃える。
...
6. import path を更新し、CSS module も移動先コンポーネントと同じ粒度の `index.module.css` にする。
...
- Atomic Design 配下で `ComponentName.module.css` を新規作成しない。CSS module は `index.module.css` にする。
