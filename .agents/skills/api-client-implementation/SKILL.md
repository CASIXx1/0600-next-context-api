...
1. 必要な request 関数を `src/requests` に追加または更新する。
   ...
   - request は `ProjectsClient`、`TasksClient` のような resource client から実行できる形にする。
   ...
   - resource client は必要に応じて `AbortController` を持ち、`abort()` method を公開する。
   - `src/contexts` には AbortController や signal を直接置かず、Application 層は state 反映とエラー表示の判断に集中させる。
...
3. request 関数を呼び出す非同期処理は `async` / `await` と `try` / `catch` で書く。
   ...
   - cleanup 関数に `client.abort()` が見えている形を優先し、useEffect 側から通信の中断タイミングを読み取れるようにする。
