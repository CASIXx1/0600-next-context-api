...
- このプロジェクトでは、API 実行と API 結果の state 反映は必ず `src/contexts` の Context API / custom hook 経由にする。
...
1. 必要な request 関数を `src/requests` に追加または更新する。
   ...
   - 新しい HTTP method が必要な場合は、resource client で `fetch` を直接呼ばず、先に `HttpRequester` に `post` / `delete` など method 別 helper を追加する。
   ...
   - 詳細画面で使う API response の field は schema に含める。UI が `createdAt`、`updatedAt`、`description` などを使うなら entity 型にも反映する。
   - request result 型と生成 helper は `src/requests/result.ts` に置く。
   - AbortError 判定 helper も `src/requests/result.ts` に置く。
   - `AbortableRequestResult` は `src/requests/result.ts` の helper で生成する。
...
2. 画面側の呼び出しを request 関数へ差し替える。
   - `src/app` や `src/components` から `src/requests` の client を直接生成しない。
   - API/state 由来の props は prop drilling せず、`src/contexts` に Provider と consumer hook を追加して受け渡す。
   - Next.js の route params/search params、汎用 UI 部品の表示 props、コンポーネント内だけで閉じる一時 UI 状態は props のままでよい。
   - 作成フォームなど presentation component はフォーム値だけを扱い、`kind` や `children` のような API payload 固有の固定値は context hook 側で補完する。
3. request 関数を呼び出す非同期処理は `async` / `await` と `try` / `catch` で書く。
   ...
   - React hooks lint が state 更新を含む callback の `useEffect` 内同期呼び出しを警告する場合は、初回 fetch は effect 内の async 関数で直接実装し、callback はイベント購読やユーザー操作用に分ける。
...
4. 成功時、失敗時の state 更新を Application 層でつなぐ。
   - 一覧から遷移する詳細画面は、route param だけで表示内容を作らず、`fetch<Resource>` など id 指定の request と context hook で実データを取得する。
   - create 成功時に API が `{ data: null }` を返す場合は、存在しない entity を捏造して state に追加しない。対象一覧を refetch するか、scoped event で invalidation して再取得する。
   - update 成功時に API が更新後 entity を返す場合は、詳細 state と必要な一覧 state に反映する。画面都合の success/error message や updating flag は `src/requests` の schema ではなく context 側に置く。
   - delete 成功時は対象一覧を invalidation/refetch し、詳細画面からは削除済み resource に留まらない navigation を行う。
   - フォーム submit では `FormData` の文字列を API boundary 型へ明示的に変換する。画面表示用の日付が `YYYY/MM/DD` の場合は、送信前に API 形式の `YYYY-MM-DD` へ正規化する。
