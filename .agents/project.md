...
- プレゼンテーション層は `src/app`、`src/components`、`src/contexts` に置く。
- 画面固有の条件値、エラー表示、API 結果の state 反映はフロントエンドのアプリケーション層の責務とする。
- API client / infrastructure 境界は `src/requests` に置く。
- API backend は `src/app/api` 配下に置く。
- 依存方向は原則として `presentation -> requests -> API endpoint -> datastore/model` とする。逆方向の依存や、UI から datastore/model への直接参照は避ける。
- `src/requests` には画面固有の固定値を持たせない。
- 更新系 API は、成功時に API レスポンスを state に反映する。楽観的更新は明示的に判断する。
- API レスポンス型は request 境界の近くに置く。
